...
public class DocenteView extends JFrame {
    ...
    private static class EstudiantesList extends JPanel {
        ...
        void setData(List<String> nombres){
            removeAll();
            if (nombres != null) {
                for (String n : nombres) { add(fila(n)); add(Box.createVerticalStrut(8)); }
            }
    ...
    private static class PropuestasList extends JPanel {
        PropuestasList(){ setOpaque(false); setLayout(new BoxLayout(this, BoxLayout.Y_AXIS)); }
        void setData(List<PropuestaItem> items){
            removeAll();
            if (items != null) {
                for (PropuestaItem it : items) { add(item(it)); add(Box.createVerticalStrut(8)); }
            }
            add(Box.createVerticalGlue());
            revalidate(); repaint();
        }
    ...
    private static class SubirPropuestaModal extends JPanel {
        ...
        final JComboBox<String> cbModalidad =
    new JComboBox<>(new String[]{
        ...
        private boolean validar(){
            ...
            if (cbModalidad.getSelectedIndex()<=0)    sb.append("• Selecciona una modalidad.\n");
            ...
            boolean esPractica = "Práctica profesional".equalsIgnoreCase(mod) || "Practica profesional".equalsIgnoreCase(mod);
            if (esPractica && !dfCarta.hasFile())     sb.append("• Debes adjuntar la Carta de Aceptación (PDF) para Práctica profesional.\n");
        ...
        // Habilita/inhabilita carta según modalidad
        private void actualizarCarta(){
            String mod = (String) cbModalidad.getSelectedItem();
            boolean enable = "Práctica profesional".equalsIgnoreCase(mod) || "Practica profesional".equalsIgnoreCase(mod);
            dfCarta.setEnabled(enable);
            revalidate(); repaint();
        }
        ...
        void reset(){
            tfTitulo.setText(""); cbModalidad.setSelectedIndex(0); dpFecha.clear();
